...
class carousel_widget extends WP_Widget {
	...
	public function display_carousel()
	{
		...
		wp_enqueue_script('roundAboutJquery',plugins_url('js/jquery.roundabout.js',__FILE__ ));
		wp_enqueue_script('roundAboutShapeJquery' ,plugins_url('js/jquery.roundabout-shapes.min.js',__FILE__ ));
		wp_enqueue_style('carouselWidgetCss',plugins_url('css/carouselwidget.css',__FILE__ ));
...
<h3 class="ui_tab_h3">Add Item to Carousel</h3> 
        ...
        <form class="update_form" method="POST" action="" > 
            ...
            <label class="howto"><span> *Image URL:</span> <input type="text" size="40"  name="imageurl"/></label><span class="media_library"><a href="upload.php?TB_iframe=true&width=1200&height=750" class="thickbox">Media</a></span><br/>
    ...
    <h3>Notes: </h3>
     ...
     <ol>
       <li > Use high resolution images for better result.
        <font style='color:black;font-size:x-small;'><br/>Media->Click picture copy 'File Url' OR use external image URL</font></li>
...
//function to display setting options and  table of contents most probably demo 
 function carousel_widget_admin_page()
 {
 	...
 	wp_enqueue_style('jqueryUiCss', plugins_url('css/jquery-ui.css',__FILE__ ));//jquery ui css
 	?>
...
<?php 
 ...
 function in_page_carousel()
 {
 	...
 	global $wpdb;
 	wp_enqueue_script('jquery');
 	wp_enqueue_script('easingJquery', plugins_url('js/jquery.easing.js',__FILE__ ));
 	wp_enqueue_script('roundAboutJquery', plugins_url('js/jquery.roundabout-shapes.min.js',__FILE__ ));
 	wp_enqueue_script('roundAboutShapeJquery',plugins_url('js/jquery.roundabout.js',__FILE__ ));
 	wp_enqueue_style('carouselWidgetCss', plugins_url('css/carouselwidget.css',__FILE__ ));
