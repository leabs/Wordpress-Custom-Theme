...
class Team implements lib\ShortcodeInterface {
	...
	public function vcMap() {
		...
		for ( $x = 1; $x < 6; $x ++ ) {
			$teamIconCollections = innovio_mikado_icon_collections()->getCollectionsWithSocialIcons();
			foreach ( $teamIconCollections as $collection_key => $collection ) {
				
				$team_social_icons_array[] = array(
					'type'       => 'dropdown',
					'heading'    => esc_html__( 'Social Icon ', 'innovio-core' ) . $x,
			...
			$team_social_icons_array[] = array(
				'type'       => 'textfield',
				'heading'    => esc_html__( 'Social Icon ', 'innovio-core' ) . $x . esc_html__( ' Link', 'innovio-core' ),
				'param_name' => 'team_social_icon_' . $x . '_link',
				'dependency' => array(
					'element' => 'team_social_icon_pack',
					'value'   => innovio_mikado_icon_collections()->getIconCollectionsKeys()
				)
			);
			
			$team_social_icons_array[] = array(
				'type'       => 'dropdown',
				'heading'    => esc_html__( 'Social Icon ', 'innovio-core' ) . $x . esc_html__( ' Target', 'innovio-core' ),
				'param_name' => 'team_social_icon_' . $x . '_target',
				'value'      => array(
					esc_html__( 'Same Window', 'innovio-core' ) => '_self',
					esc_html__( 'New Window', 'innovio-core' )  => '_blank'
				),
		...
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Team', 'innovio-core' ),
					'base'                      => $this->base,
					...
					'params'                    => array_merge(
						array(
							array(
								...
								'value'       => array(
									esc_html__( 'Info Below Image', 'innovio-core' )    => 'info-below-image',
									esc_html__( 'Info On Image Hover', 'innovio-core' ) => 'info-on-image'
								),
								'save_always' => true
							),
                            array(
                                'type'        => 'dropdown',
                                'param_name'  => 'subtype',
                                'heading'     => esc_html__( 'Hover type', 'innovio-core' ),
                                'value'       => array(
                                    esc_html__( 'Color overlay', 'innovio-core' )    => 'color-overlay',
                                    esc_html__( 'Hover Image', 'innovio-core' ) => 'hover-image'
                                ),
                            ...
                            array(
                                ...
                                'dependency' => array('element' => 'subtype', 'value' => 'hover-image'),
                            ),
							array(
								'type'       => 'textfield',
								'param_name' => 'team_name',
								'heading'    => esc_html__( 'Name', 'innovio-core' )
							),
							array(
								'type'        => 'dropdown',
								'param_name'  => 'team_name_tag',
								'heading'     => esc_html__( 'Name Tag', 'innovio-core' ),
							...
							array(
								...
								'heading'    => esc_html__( 'Name Color', 'innovio-core' ),
								'dependency' => array( 'element' => 'team_name', 'not_empty' => true )
							),
							array(
								'type'       => 'textfield',
								'param_name' => 'team_position',
								'heading'    => esc_html__( 'Position', 'innovio-core' )
							),
							array(
								'type'       => 'colorpicker',
								'param_name' => 'team_position_color',
								'heading'    => esc_html__( 'Position Color', 'innovio-core' ),
							...
							array(
								...
								'heading'     => esc_html__( 'Social Icon Pack', 'innovio-core' ),
								'value'       => array_merge( array( '' => '' ), innovio_mikado_icon_collections()->getIconCollectionsVCExclude( 'linea_icons' ) ),
								'save_always' => true
							),
						),
	...
	public function render( $atts, $content = null ) {
		...
		for ( $x = 1; $x <= $number_of_social_icons; $x ++ ) {
			...
			foreach ( innovio_mikado_icon_collections()->iconCollections as $collection_key => $collection ) {
		...
		$html = innovio_core_get_shortcode_module_template_part( 'templates/' . $params['type'], 'team', '', $params );
	...
	private function getHolderClasses( $params ) {
        ...
        $holderClasses[] = ! empty( $params['skin'] ) ? 'mkdf-team-' . esc_attr( $params['skin'] ) . '-skin' : '';

		if($params['type'] === 'info-on-image') {
		    $holderClasses[] = ! empty( $params['subtype'] ) ? 'mkdf-team-' . $params['subtype'] : '';
        }
	...
	private function getTeamSocialIcons( $params ) {
		...
		if ( $team_social_icon_pack !== '' ) {
			...
			$icon_pack                    = innovio_mikado_icon_collections()->getIconCollection( $team_social_icon_pack );
			...
			for ( $i = 1; $i <= $params['number_of_social_icons']; $i ++ ) {
				...
				if ( $team_social_icon !== '' ) {
					...
					$social_icons[] = innovio_mikado_execute_shortcode( 'mkdf_icon', $team_icon_params );
				}
			}
		}
