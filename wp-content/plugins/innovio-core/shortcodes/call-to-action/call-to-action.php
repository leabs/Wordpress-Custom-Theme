<?php
namespace InnovioCore\CPT\Shortcodes\CallToAction;
...
class CallToAction implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Call To Action', 'innovio-core' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						...
						array(
							'type'        => 'dropdown',
							'param_name'  => 'layout',
							'heading'     => esc_html__( 'Layout', 'innovio-core' ),
							'value'       => array(
						...
						array(
							...
							'heading'    => esc_html__( 'Set Content In Grid', 'innovio-core' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Content Elements Proportion', 'innovio-core' ),
							'value'      => array(
								esc_html__( '80/20', 'innovio-core' ) => '80',
								esc_html__( '75/25', 'innovio-core' ) => '75',
								esc_html__( '66/33', 'innovio-core' ) => '66',
								esc_html__( '50/50', 'innovio-core' ) => '50'
							),
							'dependency' => array( 'element' => 'layout', 'value' => array( 'normal' ) )
						),
						array(
						...
						array(
							...
							'heading'    => esc_html__( 'Button Top Margin (px)', 'innovio-core' ),
						...
						array(
							...
							'value'      => esc_html__( 'I am test text for Call to Action shortcode content', 'innovio-core' )
						)
					)
				)
			);
	...
	public function render( $atts, $content = null ) {
		...
		$html = innovio_core_get_shortcode_module_template_part( 'templates/call-to-action', 'call-to-action', '', $params );
	...
	private function getButtonHolderStyles( $params ) {
		...
		if ( ! empty( $params['button_top_margin'] ) && $params['layout'] === 'simple' ) {
			$styles[] = 'margin-top: ' . innovio_mikado_filter_px( $params['button_top_margin'] ) . 'px';
		}
	...
	private function getButtonParameters( $params ) {
		...
		if ( ! empty( $params['button_hover_shadow'] ) ) {
			$button_params_array['hover_shadow'] = $params['button_hover_shadow'];
		}
