<?php
namespace InnovioCore\CPT\Shortcodes\ProductInfo;
...
class ProductInfo implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if(function_exists('vc_map')) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Product Info', 'innovio' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						array(
							...
							'heading'     => esc_html__( 'Selected Product', 'innovio' ),
							'settings'    => array(
								'sortable'      => true,
								'unique_values' => true
							),
							'admin_label' => true,
							'save_always' => true,
							'description' => esc_html__( 'If you left this field empty then product ID will be of the current page', 'innovio' )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'product_info_type',
							'heading'     => esc_html__( 'Product Info Type', 'innovio' ),
							'value'       => array(
								esc_html__( 'Title', 'innovio' )          => 'title',
								esc_html__( 'Featured Image', 'innovio' ) => 'featured_image',
								esc_html__( 'Category', 'innovio' )       => 'category',
								esc_html__( 'Excerpt', 'innovio' )        => 'excerpt',
								esc_html__( 'Price', 'innovio' )          => 'price',
								esc_html__( 'Rating', 'innovio' )         => 'rating',
								esc_html__( 'Add To Cart', 'innovio' )    => 'add_to_cart',
								esc_html__( 'Tag', 'innovio' )            => 'tag',
								esc_html__( 'Author', 'innovio' )         => 'author',
								esc_html__( 'Date', 'innovio' )           => 'date',
							),
							'admin_label' => true
						),
						array(
						...
						array(
							...
							'description' => esc_html__( 'Set title tag for product title element', 'innovio' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Featured Image Proportions', 'innovio' ),
							'value'      => array(
								esc_html__( 'Default', 'innovio' )        => '',
								esc_html__( 'Original', 'innovio' )       => 'full',
								esc_html__( 'Square', 'innovio' )         => 'innovio_mikado_image_square',
								esc_html__( 'Landscape', 'innovio' )      => 'innovio_mikado_image_landscape',
								esc_html__( 'Portrait', 'innovio' )       => 'innovio_mikado_image_portrait',
								esc_html__( 'Medium', 'innovio' )         => 'medium',
								esc_html__( 'Large', 'innovio' )          => 'large',
								esc_html__( 'Shop Single', 'innovio' )    => 'woocommerce_single',
								esc_html__( 'Shop Thumbnail', 'innovio' ) => 'woocommerce_thumbnail'
							),
						...
						array(
							...
							'heading'     => esc_html__( 'Category Tag', 'innovio' ),
							'value'       => array_flip( innovio_mikado_get_title_tag( true ) ),
							'description' => esc_html__( 'Set category tag for product category element', 'innovio' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Add To Cart Skin', 'innovio' ),
							'value'      => array(
								esc_html__( 'Default', 'innovio' ) => '',
								esc_html__( 'White', 'innovio' )   => 'white',
								esc_html__( 'Dark', 'innovio' )    => 'dark'
							),
						...
						array(
							...
							'heading'    => esc_html__( 'Add To Cart Size', 'innovio' ),
							'value'      => array(
								esc_html__( 'Default', 'innovio' ) => '',
							...
							'dependency' => array( 'element' => 'product_info_type', 'value'   => array( 'innovio' ) ),
							'group'      => esc_html__( 'Product Info Style', 'innovio' )
						)
					)
				)
	...
	public function getItemTitleHtml( $params ) {
		...
		if ( ! empty( $title ) ) {
			$html = '<' . esc_attr( $title_tag ) . ' itemprop="name" class="mkdf-pi-title entry-title" ' . innovio_mikado_get_inline_style( $product_info_style ) . '>';
			...
			$html .= '<div class="mkdf-pi-category">';
				foreach ( $categories as $cat ) {
					if ( ! empty( $params['category_tag'] ) ) {
						$html .= '<' . esc_attr( $params['category_tag'] ) . ' ' . innovio_mikado_get_inline_style( $product_info_style ) . '>';
						...
						$html .= '<a itemprop="url" class="mkdf-pi-category-item" href="' . esc_url( get_term_link( $cat->term_id ) ) . '" ' . innovio_mikado_get_inline_style( $product_info_style ) . '>' . esc_html( $cat->name ) . '</a>';
					}
				}
			$html .= '</div>';
		}
	...
	public function getItemExcerptHtml( $params ) {
		...
		if ( ! empty( $excerpt ) ) {
			$html = '<div class="mkdf-pi-excerpt"><p itemprop="description" class="mkdf-pi-excerpt-item" ' . innovio_mikado_get_inline_style( $product_info_style ) . '>' . esc_html( $excerpt ) . '</p></div>';
		}
	...
	public function getItemPriceHtml( $params ) {
		...
		if ( $price_html = $product->get_price_html() ) {
			$html = '<div class="mkdf-pi-price" ' . innovio_mikado_get_inline_style( $product_info_style ) . '>' . $price_html . '</div>';
		}
	...
	public function getItemRatingHtml( $params ) {
		...
		if ( get_option( 'woocommerce_enable_review_rating' ) !== 'no' ) {
			...
			$html = '<div class="mkdf-pi-rating" title="' . sprintf( esc_attr__( "Rated %s out of 5", "innovio" ), $average ) . '" ' . innovio_mikado_get_inline_style( $product_info_style ) . '><span style="width:' . ( ( $average / 5 ) * 100 ) . '%"></span></div>';
		}
	...
	public function getItemAddToCartHtml( $params ) {
		...
		$html .= apply_filters( 'innovio_mikado_filter_product_info_add_to_cart_link',
	...
	public function getItemTagHtml( $params ) {
		...
		if ( ! empty( $tags ) ) {
			$html = '<div class="mkdf-pi-tag">';
				foreach ( $tags as $tag ) {
					$html .= '<a itemprop="url" class="mkdf-pi-tag-item" href="' . esc_url( get_term_link( $tag->term_id ) ) . '" ' . innovio_mikado_get_inline_style( $product_info_style ) . '>' . esc_html( $tag->name ) . '</a>';
				}
			$html .= '</div>';
		}
	...
	public function getItemAuthorHtml( $params ) {
		...
		if ( ! empty( $author ) ) {
			$html = '<div class="mkdf-pi-author" ' . innovio_mikado_get_inline_style( $product_info_style ) . '>' . esc_html( $author ) . '</div>';
		}
	...
	public function getItemDateHtml( $params ) {
		...
		if ( ! empty( $date ) ) {
			$html = '<div class="mkdf-pi-date" ' . innovio_mikado_get_inline_style( $product_info_style ) . '>' . esc_html( $date ) . '</div>';
		}
	...
	public function productIdAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				$data = array();
				$data['value'] = $value['id'];
				$data['label'] = esc_html__( 'Id', 'innovio' ) . ': ' . $value['id'] . ( ( strlen( $value['title'] ) > 0 ) ? ' - ' . esc_html__( 'Title', 'innovio' ) . ': ' . $value['title'] : '' );
				$results[] = $data;
			}
		}
		...
		if ( ! empty( $query ) ) {
			...
			if ( ! is_wp_error( $product ) ) {
				...
				if ( ! empty( $product_title ) ) {
					$product_title_display = ' - ' . esc_html__( 'Title', 'innovio' ) . ': ' . $product_title;
				}
				
				$product_id_display = esc_html__( 'Id', 'innovio' ) . ': ' . $product_id;
