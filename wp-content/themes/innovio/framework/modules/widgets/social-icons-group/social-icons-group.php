...
if ( class_exists( 'InnovioCoreClassWidget' ) ) {
    class InnovioMikadoClassClassIconsGroupWidget extends InnovioCoreClassWidget {
        public function __construct() {
            parent::__construct(
                'mkdf_social_icons_group_widget',
                esc_html__( 'Innovio Social Icons Group Widget', 'innovio' ),
                array( 'description' => esc_html__( 'Use this widget to add a group of up to 6 social icons to a widget area.', 'innovio' ) )
            );
        ...
        protected function setParams() {
            $this->params = array_merge(
                ...
                innovio_mikado_icon_collections()->getSocialIconWidgetMultipleParamsArray(6),
                array(
                    array(
                        'type'    => 'dropdown',
                        'name'    => 'alignment',
                        'title'   => esc_html__( 'Text Alignment', 'innovio' ),
                        'options' => array(
                    ...
                    array(
                        'type'  => 'textfield',
                        'name'  => 'icon_size',
                        'title' => esc_html__( 'Icons Size (px)', 'innovio' )
                    ),
                    array(
                        'type'  => 'colorpicker',
                        'name'  => 'color',
        ...
        public function widget( $args, $instance ) {
            ...
            $hover_background_color = ! empty( $instance['hover_background_color'] ) ? $instance['hover_background_color'] : '';
            $class = implode( ' ', $class );
            ...
            for ( $n = 1; $n <= 6; $n++ ) {
                ...
                <?php if ( ! empty( $icon_holder_html ) ) { ?>
                    <a class="mkdf-social-icon-widget-holder mkdf-icon-has-hover" <?php echo innovio_mikado_get_inline_attr( $hover_background_color, 'data-hover-background-color' ); ?> <?php innovio_mikado_inline_style( $icon_styles ) ?>
